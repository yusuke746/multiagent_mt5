...
## TradingAgents のアナリスト構成

| アナリスト    | 分析内容                          | .env キー              |
| ------------ | -------------------------------- | ---------------------- |
| market       | テクニカル指標 (MACD/RSI等)       | TA_USE_MARKET=true     |
| news         | グローバルニュース・マクロ経済     | TA_USE_NEWS=true       |
| social       | StockTwits/Reddit センチメント    | TA_USE_SOCIAL=true     |
| fundamentals | ファンダメンタルズ (Gold には限定的) | TA_USE_FUNDAMENTALS=false |
...
TradingAgents はソースからインストールが必要です:

```powershell
# TradingAgents 本体を先にインストール
git clone https://github.com/TauricResearch/TradingAgents.git ..\TradingAgents
pip install ..\TradingAgents

# システム依存パッケージ
pip install MetaTrader5 pywin32 python-dotenv requests numpy pandas
```
...
| ------------------------- | -------------------------------- | --------- |
| SYMBOLS                   | 監視銘柄 (カンマ区切り)           | GOLD      |
| RISK_PER_TRADE            | 1トレードあたりリスク率            | 0.01 (1%) |
| ENTRY_SL_ATR_MULT_GOLD    | GOLDのSL倍率 (ATR×倍率)          | 2.0       |
| ENTRY_TP_R                | TP = SL距離 × この倍率            | 2.0 (2R)  |
| TA_USE_MARKET             | テクニカルアナリスト有効           | true      |
| TA_USE_NEWS               | ニュースアナリスト有効             | true      |
| TA_USE_SOCIAL             | センチメントアナリスト有効         | true      |
| TA_USE_FUNDAMENTALS       | ファンダメンタルズアナリスト有効    | false     |
| TA_DEEP_MODEL             | 深い推論に使うモデル              | gpt-4.1   |
| TA_QUICK_MODEL            | 高速タスクに使うモデル             | gpt-4.1-mini |
| TA_EXIT_ON_SIGNAL_FLIP    | シグナル反転時にポジション決済     | true      |

## 注意事項

- 本システムは**デモ口座での検証**を目的としています。実運用前に十分なテストを行ってください。
- TradingAgents の分析は **非決定的** です (同じ日付でも毎回異なる結果になりえます)。
- Gold (GC=F) は米国先物市場のデータを参照するため、MT5 のスポット価格と若干乖離があります。
- APIコスト: TradingAgents は1分析につき LLM を複数回呼び出します (1時間ごと)。
...
├── main.py           # メインループ (H1+M15サイクル)
├── config.py         # 設定 (.envから読み込み)
├── ta_analyzer.py    # TradingAgents統合 [新規]
├── symbol_map.py     # MT5→yfinanceシンボルマッピング [新規]
├── mt5_connector.py  # MT5接続・発注
├── risk_manager.py   # 相関リスク制御
├── lot_calculator.py # ロット計算
├── discord_notifier.py # Discord通知
├── trade_logger.py   # SQLite記録
├── news_monitor.py   # ニュース監視
├── market_stress.py  # 市場ストレス検知
├── .env.example      # 設定テンプレート
└── requirements.txt
```
